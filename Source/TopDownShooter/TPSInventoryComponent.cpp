...
bool UTPSInventoryComponent::SwitchWeaponToIndex(int32 ChangeToIndex, int32 OldIndex, FAddicionalWeaponInfo OldInfo, bool bIsForward)
{
	...
	if (WeaponSlots.IsValidIndex(CorrectIndex))
	{
		if (!WeaponSlots[CorrectIndex].NameItem.IsNone())
		{
			...
			if (bIsSuccess)
			{
				...
				NewAdditionalInfo = WeaponSlots[CorrectIndex].AddicionalInfo;
			}
		}
	}
		...
		if (bIsForward)
		{
			...
			while (iteration < WeaponSlots.Num() && !bIsSuccess)
			{
				...
				if (WeaponSlots.IsValidIndex(tmpIndex))
				{
					...
					{
						if (WeaponSlots[tmpIndex].AddicionalInfo.Round > 0)
						{

							bIsSuccess = true;
							NewIdWeapon = WeaponSlots[tmpIndex].NameItem;
							NewAdditionalInfo = WeaponSlots[tmpIndex].AddicionalInfo;
				...
				else
				{
					...
					if (OldIndex != Seconditeration)
					{
						if (WeaponSlots.IsValidIndex(Seconditeration))
						{
							...
							{
								if (WeaponSlots[Seconditeration].AddicionalInfo.Round > 0)
								{
									//WeaponGood
									bIsSuccess = true;
									NewIdWeapon = WeaponSlots[Seconditeration].NameItem;
									NewAdditionalInfo = WeaponSlots[Seconditeration].AddicionalInfo;
...
bool UTPSInventoryComponent::CheckAmmoForWeapon(EWeaponType TypeWeapon,int8 &AvailbleAmmoForWeapon)
{
	AvailbleAmmoForWeapon = 0;
	bool bIsFind = false;
	int8 i = 0;
		...
		while (i < AmmoSlots.Num() && !bIsFind)
		{
			...
			{
				bIsFind = true;
				AvailbleAmmoForWeapon = AmmoSlots[i].Cout;
...
bool UTPSInventoryComponent::SwitchWeaponToInventory(FWeaponSlot NewWeapon, int32 IndexSlot, int32 CurrentIndexWeaponChar, FDropItem& DropItemInfo)
{
	...
	if (WeaponSlots.IsValidIndex(IndexSlot) && GetDropItemInfoFromInventory(IndexSlot, DropItemInfo))
	{
		...
		SwitchWeaponToIndex(CurrentIndexWeaponChar,-1,NewWeapon.AddicionalInfo,true);
